...
- Publications and working papers are lists of `<li>` entries, each with a title link, author/venue line, and a collapsible `<details>/<summary>` block holding the abstract. Follow this existing `<li>`/`<details>` pattern when adding new entries rather than introducing new markup styles. Publications is a numbered `<ol reversed>` (highest number first); Working Papers and all other lists on the page use plain bullets (`<ul>`), not numbering.
