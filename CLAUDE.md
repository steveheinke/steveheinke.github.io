...
- The page is structured as stacked sections identified by anchor `id`s, linked from a sticky dropdown menu (`.menu-dropdown` / `.menu-content`) near the top: `about-me`, `publications`, `working-papers`, `research-in-progress-selected`, `professional-services-selected`, `teaching-experience`. The dropdown open/close behavior is pure CSS (`:hover`), with no JavaScript in the page.
- Publications and working papers are `<ol>` lists of `<li>` entries, each with a title link, author/venue line, and a collapsible `<details>/<summary>` block holding the abstract. Follow this existing `<li>`/`<details>` pattern when adding new entries rather than introducing new markup styles.
