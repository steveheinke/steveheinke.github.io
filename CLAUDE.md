...
- Publications and working papers are lists of `<li>` entries, each with a title link, author/venue line, and a collapsible `<details>/<summary>` block holding the abstract. Follow this existing `<li>`/`<details>` pattern when adding new entries rather than introducing new markup styles. Publications is a numbered `<ol reversed start="9">` (highest number first); the `start` value is hardcoded to the current publication count (not browser-auto-computed, which is unreliable across browsers) — update it whenever a publication is added or removed. Working Papers and all other lists on the page use plain bullets (`<ul>`), not numbering.
